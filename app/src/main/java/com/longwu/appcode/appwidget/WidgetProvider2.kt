...
class WidgetProvider2 : AppWidgetProvider() {
    ...
    private val TAG: String = WidgetProvider2::class.java.simpleName
    ...
    override fun onUpdate(
        ...
        appWidgetIds?.also {
            Log.d(TAG, "onUpdate >> appWidgetIds length : " + it.size)
            try {
                for (appWidgetId in it) {
                    Log.i(TAG, "appWidgetId=$appWidgetId")
                    val remoteViews =
                        RemoteViews(context?.packageName, R.layout.layout_desktop_widget2)
